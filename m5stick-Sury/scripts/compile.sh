...
arduino-cli compile --log \
  ...
  --output-dir ./m5stick-sury/build \
  ./m5stick-sury/m5stick-sury.ino

if [[ "${locale}" == "en-us" ]]; then
  output_file="M5sury-${name}.bin"
else
  output_file="M5sury-${name}.${locale}.bin"
fi

echo "Merging binaries to ${output_file}"

esptool.py --chip esp32s3 merge_bin --output ./m5stick-sury/build/${output_file}  \
            ${bootloader_addr} ./m5stick-sury/build/m5stick-sury.ino.bootloader.bin \
            0x8000 ./m5stick-sury/build/m5stick-sury.ino.partitions.bin \
            0x10000 ./m5stick-sury/build/m5stick-sury.ino.bin
